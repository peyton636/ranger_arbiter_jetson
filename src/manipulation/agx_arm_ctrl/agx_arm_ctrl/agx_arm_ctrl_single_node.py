...
class AgxArmRosNode(Node):
    ...
    def _declare_parameters(self):
        ...
        self.declare_parameter(
            "accept_joint_states_arm_control", True,
        )
    ...
    def _load_parameters(self):
        ...
        self.accept_joint_states_arm_control = self.get_parameter(
            "accept_joint_states_arm_control").value
    ...
    def _setup_subscribers(self):
        ...
        if self.gripper is not None:
            # 夹爪专用控制话题，避免与仿真 joint_state_broadcaster 争抢 /control/joint_states
            self.create_subscription(
                JointState, "control/gripper_joint_states",
                self._gripper_joint_states_callback, 10,
                callback_group=self._gripper_cb_group,
            )
            self.get_logger().info(
                "Subscribed control/gripper_joint_states for gripper commands"
            )
        if self.hand is not None:
            self.create_subscription(
    ...
    def _sanitize_joint_value(self, value) -> float:
        if not isinstance(value, (int, float)) or math.isnan(value) or math.isinf(value):
            return 0.0
        return float(value)
    ...
    def _check_can_control_gripper(self) -> bool:
        """夹爪控制门控：不依赖 accept_joint_states_arm_control。"""
        ...
        if not self.enable_flag:
            self.get_logger().warn("Agx_arm is not enabled, cannot control gripper")
            return False
        return True
    ...
    def _get_gripper_joint_data(self):
        ...
        return [(GRIPPER_JOINT_NAME, self._sanitize_joint_value(status.width), 0.0, self._sanitize_joint_value(status.force))]
...
#ros控制节点，解析（effort字段）effort力
    ...
    def _joint_states_callback(self, msg: JointState):
        ...
        has_arm_joint = any(name in self.arm_joint_names for name in msg.name)
        has_gripper = GRIPPER_JOINT_NAME in msg.name

        if has_arm_joint and self.accept_joint_states_arm_control:
            self._control_arm_joints(joint_pos)
        # 仿真 joint_state_broadcaster 会带全关节；仅含 gripper 的单关节消息仍走此路径（手动调试）
        if has_gripper and (
            self.gripper is None
            or (len(msg.name) == 1 and msg.name[0] == GRIPPER_JOINT_NAME)
        ):
            self._control_gripper_joint(joint_pos, joint_effort)
        self._control_hand_joints(joint_pos)

    def _gripper_joint_states_callback(self, msg: JointState):
        """夹爪专用控制（供 gripper_controller_node 使用，避免仿真话题抢占）。"""
        if not self._check_can_control_gripper():
            return
