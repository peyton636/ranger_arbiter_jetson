...
MAX_STEER_SIDEWAYS_MILLIRAD = 1570  # 90° 斜移，与 MCU ARB_SLIDE_ANGLE_90 一致
# Back-compat alias
...
def twist_to_motion(
    ...
    *,
    ...
    sideways_steer_invert: bool = False,
) -> MotionCommand:
    """
    Map ROS /cmd_vel → V3 / 0x111（先由 MCU 切 0x141 motion_model，再周期发 0x111）。

    优先级：linear.y（斜移）> angular.z（自旋）> linear.x（阿克曼直走）。

    | 意图 | motion_model | v | steer | omega |
    |------|--------------|---|-------|-------|
    | 前进/后退 | 0 阿克曼 | ±|lx| | 0 | 0 |
    | 左/右平移 | 1 斜移 | +|ly| | 左+1570 右-1570 | 0 |
    | 原地转 | 2 自旋 | 0 | 0 | ±az |
    """
    eps = 0.02
    ...
    # 斜移 0x141=1：MCU 约定 左 steer≥+80(+1570)、右 steer≤-80(-1570)
    if abs(ly) > eps:
        steer = steer_90 if ly > 0 else -steer_90
        if sideways_steer_invert:
            steer = -steer
        v_mm_s = _clamp_v_for_steer(int(round(abs(ly) * 1000.0)), abs(steer))
    ...
    # 前后直走：0x141=0x00，steer/omega 均为 0
    v_mm_s, _ = twist_to_cmd(lx, 0.0)
    return MotionCommand(
        v_mm_s=v_mm_s,
        omega_millirad_s=0,
        steer_millirad=0,
