...
from eth_gateway.blob_time_stamp import mcu_virtual_to_ros_stamp
from eth_gateway.time_stamp import estimate_utc_sec, mono_ms, utc_sec_to_stamp
...
class GpsFixHelper:
    ...
    def on_time_sync(self, msg: TimeSyncResponse) -> None:
        ...
        if msg.utc_unix_sec > 0:
            ...
            self._logger.info(
                f"TimeSync QUERY ��??: utc={self._utc_anchor_sec} "
    ...
    def check_stale(self) -> None:
        if self._last_gps_rx_time <= 0:
            self._logger.warn(
                "??��??? MCU GPS ??????????��?????? F407 ?? GPS fix",
                throttle_duration_sec=10.0,
            )

    def _observation_ros_stamp(self):
        stamps = []
        if self._latest_a is not None:
            stamps.append(self._latest_a.header.stamp)
        if self._latest_b is not None:
            stamps.append(self._latest_b.header.stamp)
        if self._latest_c is not None:
            stamps.append(self._latest_c.header.stamp)
        if stamps:
            return max(stamps, key=lambda s: float(s.sec) + s.nanosec * 1e-9)
        return None

    def _stamp_for_observation(self):
        if not self._use_time_sync_stamp:
            return self._node.get_clock().now().to_msg()
        obs = self._observation_ros_stamp()
        if obs is not None:
            return obs
        ...
        return mcu_virtual_to_ros_stamp(
            self._offset_ms, True, self._node.get_clock()
        )
    ...
    def _log_fix(
        ...
        if self._utc_anchor_sec > 0:
            utc_note = f", stamp_sec={stamp_sec}(UTC��??)"
        elif self._use_time_sync_stamp and self._time_sync_ready:
            utc_note = ", stamp=????MCU???(??UTC��??)"

        if pos_valid and not math.isnan(lat) and not math.isnan(lon):
            if not self._had_fix:
                self._logger.info(
                    f"GPS fix ??��: lat={lat:.7f}, lon={lon:.7f}, ????={num_sv}{utc_note}"
                )
                self._had_fix = True
        else:
            self._logger.info(
                f"GPS ?? fix: ????={num_sv}??????? /fix status=NO_FIX",
                throttle_duration_sec=10.0,
            )
            self._had_fix = False
