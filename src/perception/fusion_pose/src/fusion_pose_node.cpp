...
namespace perception
{
    ...
    {
        // 2D 检测结果输入话题，要求发布 vision_msgs::msg::Detection2DArray 消息，且消息中的 bbox.center 与 bbox.size_x/y 能正确反映检测框中心和尺寸
        detections_topic_ = this->declare_parameter<std::string>(
            "detections_topic", "/perception/detect/detections_2d");

        // 深度图输入话题，要求与 detections_topic_ 同步，且对应同一时间戳的消息能正确转换为 OpenCV 图像
    ...
    FusionPoseNode::CallbackReturn FusionPoseNode::on_configure(const rclcpp_lifecycle::State &)
    {
        ...
        // 输出目标位姿数组，消息类型 geometry_msgs::msg::PoseArray，位姿相对于 target_frame_ 坐标系
        if (!pose_pub_)
    ...
    FusionPoseNode::CallbackReturn FusionPoseNode::on_activate(const rclcpp_lifecycle::State &)
    {
        ...
        const rmw_qos_profile_t camera_info_qos =
            rclcpp::QoS(rclcpp::KeepLast(10)).reliable().durability_volatile().get_rmw_qos_profile();
        ...
        sync_ = std::make_shared<message_filters::Synchronizer<
            message_filters::sync_policies::ApproximateTime<DetectionArray, ImageMsg, CameraInfoMsg>>>(
            message_filters::sync_policies::ApproximateTime<DetectionArray, ImageMsg, CameraInfoMsg>(20),
            detections_sub_, depth_sub_, camera_info_sub_);

        sync_->setMaxIntervalDuration(rclcpp::Duration::from_seconds(0.1)); // 设置最大时间间隔，超过则不匹配
        RCLCPP_INFO(get_logger(), "sync created, max interval = 0.1s");
        // 注册同步回调，当 detections_msg、depth_msg、camera_info_msg 三者在时间上近似同步时被调用
        sync_->registerCallback(
            std::bind(
                &FusionPoseNode::syncCallback,
                this,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3));
    ...
    FusionPoseNode::CallbackReturn FusionPoseNode::on_deactivate(const rclcpp_lifecycle::State &)
    {
    ...
    FusionPoseNode::CallbackReturn FusionPoseNode::on_cleanup(const rclcpp_lifecycle::State &)
    {
    ...
    FusionPoseNode::CallbackReturn FusionPoseNode::on_shutdown(const rclcpp_lifecycle::State &state)
    {
        RCLCPP_INFO(
            get_logger(),
    ...
    void FusionPoseNode::syncCallback(const DetectionArray::ConstSharedPtr &detections_msg,
                                      const ImageMsg::ConstSharedPtr &depth_msg,
                                      const CameraInfoMsg::ConstSharedPtr &camera_info_msg)
    {
        // RCLCPP_INFO(this->get_logger(), "syncCallback called with %zu detections", detections_msg->detections.size());
        // 从相机内参中读取 fx, fy, cx, cy
    ...
    // 从检测框对应区域中提取一个稳定深度值
    bool FusionPoseNode::getDepthFromDetection(const cv::Mat &depth_image,
    ...
    // 将相机坐标系下的点转换到目标坐标系
    bool FusionPoseNode::transformPointToTarget(const tf2::Vector3 &p_camera,
    ...
    tf2::Transform FusionPoseNode::transformMsgToTf(const geometry_msgs::msg::Transform &msg)
    {
        // 提取四元数并归一化
