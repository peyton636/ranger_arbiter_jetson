#include "nodes/detector_node.hpp"
...
YolosDetectorNode::CallbackReturn YolosDetectorNode::on_configure(const rclcpp_lifecycle::State&) {
  ...
  try {
    ...
    det_pub_ = create_publisher<vision_msgs::msg::Detection2DArray>("/perception/detect/detections_2d", 10);
    if (publish_debug_image_) debug_pub_ = create_publisher<sensor_msgs::msg::Image>("/perception/detect/debug_image", 1);
    if (publish_timing_) timing_pub_ = create_publisher<std_msgs::msg::Float64MultiArray>("/perception/detect/timing", 10);
...
YolosDetectorNode::CallbackReturn YolosDetectorNode::on_activate(const rclcpp_lifecycle::State&) {
  ...
  RCLCPP_INFO(get_logger(), "Activated - listening on /camera/color/image_raw");
  return CallbackReturn::SUCCESS;
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(perception::YolosDetectorNode)
