#include "nodes/segmentor_node.hpp"
#include "conversion/segmentation_converter.hpp"
...
YolosSegmentorNode::CallbackReturn YolosSegmentorNode::on_configure(const rclcpp_lifecycle::State&) {
  ...
  det_pub_ = create_publisher<vision_msgs::msg::Detection2DArray>("/perception/segment/detections", 10);
  mask_pub_ = create_publisher<sensor_msgs::msg::Image>("/perception/segment/mask", 1);
  if (debug_) debug_pub_ = create_publisher<sensor_msgs::msg::Image>("/perception/segment/debug_image", 1);
...
RCLCPP_COMPONENTS_REGISTER_NODE(perception::YolosSegmentorNode)
