...
YolosPoseNode::CallbackReturn YolosPoseNode::on_configure(const rclcpp_lifecycle::State&) {
  ...
  pub_ = create_publisher<vision_msgs::msg::Detection2DArray>("/perception/pose/detections", 10);
  if (debug_) debug_pub_ = create_publisher<sensor_msgs::msg::Image>("/perception/pose/debug_image", 1);
...
RCLCPP_COMPONENTS_REGISTER_NODE(perception::YolosPoseNode)
