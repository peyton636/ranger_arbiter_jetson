...
//UDP发送数据内容
const u8 *tcp_demo_sendbuf="PRECHIN-STM32F407 UDP demo send data\r\n";
...
void udp_demo_test(void)
{
	...
	while(res==0)
	{
		...
		if(udp_demo_flag&1<<6)//是否收到数据?
		{
			...
			LCD_ShowString(30,230,tftlcd_data.width-30,tftlcd_data.height-230,16,udp_demo_recvbuf);//显示接收到的数据			
